...
impl PageState {
    pub fn new(cx: Scope) -> Self {
        Self {
            ...
            form_erros: KeyedNotifications::default(),
        }
    }

    pub fn can_submit(&self) -> bool {
        !(self.form_erros.has_messages() || self.username.current().is_empty() || self.password.current().is_empty())
    }
}
...
pub fn Register(cx: Scope) -> Element {
    ...
    let username_oninput = sync_handler!([page_state], move |ev: FormEvent| {
        if let Err(e) = uchat_domain::Username::new(&ev.value) {
            page_state.with_mut(|state| state.form_erros.set(
                "bad-username", e.to_string()))
        } else {
            page_state.with_mut(|state| state.form_erros.remove("bad-username"));
        }
    ...
    let password_oninput = sync_handler!([page_state], move |ev: FormEvent| {
        if let Err(e) = uchat_domain::Password::new(&ev.value) {
            page_state.with_mut(|state| state.form_erros.set(
                "bad-password", e.to_string()))
        } else {
            page_state.with_mut(|state| state.form_erros.remove("bad-password"));
        }
    ...
    cx.render(rsx! {
        form {
            ...
            KeyedNotificationBox { 
                legend: "Form Errors",
                notifications: page_state.clone().with(|state| state.form_erros.clone()),
            }
